...
[![Logging](https://img.shields.io/badge/slog%20%2B%20ZeroLog-Structured%20Logging-blue)](https://github.com/samber/slog-zerolog)
...
Все REST-эндпоинты возвращают одно из двух различных тел, поэтому клиенты могут
отличить тело успеха от тела ошибки по форме. Успешный ответ — это `Ok`:
...
- `message` — краткое описание: `"ok"` при успехе.
- `data` — полезная нагрузка; `null` для операций без тела (например, удаление).

При ошибке HTTP-код сохраняется, а тело — это конверт `Error`, который никогда не содержит
поле `data`:
